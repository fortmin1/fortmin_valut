...
## 操作系统家谱
![](assets/语言基础/file-20260714150924496.png)
# 基础语法
## 数据类型
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/76icEib4KCvEMQ2jktVUHHgfLtnZoNUJPJaj9QVX7Rp0sqer5pPzrDib9AeYdkkvUK6Y3zxrJqicCI0HQibdUGuq3Gg/640?wx_fmt=jpeg&tp=wxpic&wxfrom=5&wx_lazy=1#imgIndex=4)
使用：
```C
...
```
又由于`[]`运算符的优先级高于`*`，因此圆括号可以省略，其简写形式为：
```C
dataType *arrayName[length];
```
### 数组指针
...
int (*p2)[10];
```
![](https://mmbiz.qpic.cn/mmbiz_jpg/76icEib4KCvEMQ2jktVUHHgfLtnZoNUJPJ1JE3NqahuzmD5y1XodGiaa4e3M1MtXAVAf2YwPrhOyR0IZpsAaZzvhA/640?wx_fmt=jpeg&tp=wxpic&wxfrom=5&wx_lazy=1#imgIndex=6)
### void* 指针
void指针可以指向任意类型。
指针用来保存变量的内存地址与变量的类型无关，任何类型指针都可以保存任何一个地址；
指针之所以需要类型，只与该指针的解引用有关
使用void指针可以实现泛型的效果：
```C
// 交换两个变量的值  
